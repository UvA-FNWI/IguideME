...
export const postLayoutColumns: (
  layouts: LayoutColumn[],
) => Promise<void> = async (layouts: LayoutColumn[]) => {
...
export const postTileGroup: (group: TileGroup) => Promise<void> = async (
  group: TileGroup,
) => {
  await apiClient.post(`tiles/groups/${group.id}`, group);
};

export const deleteTileGroup: (id: number) => Promise<void> = async (
  id: number,
) => {
  await apiClient.delete(`tiles/groups/${id}`);
};

export const patchTileGroup: (group: TileGroup) => Promise<void> = async (
  group: TileGroup,
) => {
  await apiClient.patch(`tiles/groups/${group.id}`, group);
};

export const patchTileGroupOrder: (ids: number[]) => Promise<void> = async (
  ids: number[],
) => {
  await apiClient.patch(`tiles/groups/order`, ids);
};

export const getGroupTiles: (id: number) => Promise<Tile[]> = async (
  id: number,
) =>
  await apiClient
    .get(`tilegroup/${id}/tiles`)
...
export const getTileGrades: (
  userID: string,
  tileID: number,
) => Promise<TileGrades> = async (userID: string, tileID: number) =>
  await apiClient
...
export const patchTileOrder: (ids: number[]) => Promise<void> = async (
  ids: number[],
) => {
