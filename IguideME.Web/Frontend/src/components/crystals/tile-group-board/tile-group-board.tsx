...
const TileGroupBoard: FC = (): ReactElement => {
      ...
      <div className="tileGroupBoard">
        ...
        >
          ...
          {createPortal(
            <DragOverlay>
              {activeGroup !== null && (
                <AdminTileGroupView group={activeGroup} />
              )}
              {activeTile !== null && (
                <AdminTileView tile={activeTile} move={move} />
              )}
            </DragOverlay>,
            document.body,
          )}
  ...
  function onDragEnd(event: DragEndEvent): void {
    ...
    switch (activeData.type) {
      ...
      default:
        console.warn(
          "Unknown active object type encountered during drop",
          activeData.type,
        );
    }
  }
  ...
  function dropTile(
    ...
    switch (overData.type) {
      case "Group":
        activeData.tile.group_id !== +overID - 1 &&
          moveTileToGroup(activeData.tile, +overID - 1);
        return;
      case "Tile":
        activeData.tile.group_id === overData.tile.group_id
          ? swapTiles(
              +(activeID as string).substring(1) - 1,
              +(overID as string).substring(1) - 1,
              overData,
            )
      ...
      default:
        console.warn(
          "Unknown over object type encountered during drop",
          activeData.type,
        );
    }
  }
  ...
  function swapTiles(activeID: number, overID: number, overData: any): void {
    const ids = tiles!.flatMap((tile) =>
      tile.group_id === overData.tile.group_id ? [tile.id] : [],
    );
    const activeIndex = ids.findIndex((id) => id === activeID);
    const overIndex = ids.findIndex((id) => id === overID);
  ...
  function moveTileToGroup(tile: Tile, gid: number): void {
    ...
    updateTile({
      ...
      group_id: gid,
      position:
        (gtiles.length > 0 ? gtiles[gtiles.length - 1].position : 0) + 1,
    });
  }
};
