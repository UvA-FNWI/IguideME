...
function entriesColumns(
  ...
  return {
    ...
    children: tileEntries.map((e) => ({
      ...
      width: 150,
      sorter: (a: any, b: any) =>
        a[tile.id + "_" + e.id] - b[tile.id + "_" + e.id],
...
function averagedEntriesColumns(tile: Tile, tileEntries: TileEntry[]) {
  return {
    ...
    width: 250,
    sorter: (a: any, b: any) => {
      const a_grades: number[] = tileEntries
        .map((e) => a[`${e.tile_id}_${e.id}`])
        .filter((x) => x.length > 0)
        .map((x) => parseFloat(x));

      const b_grades: number[] = tileEntries
        .map((e) => b[`${e.tile_id}_${e.id}`])
        .filter((x) => x.length > 0)
        .map((x) => parseFloat(x));

      if (a_grades.length > 0 && b_grades.length > 0) {
        return (
          a_grades.reduce((c, d) => c + d, 0) / a_grades.length -
    ...
    render: (text: string, row: any) => {
      const grades: number[] = tileEntries
        .map((e) => row[`${e.tile_id}_${e.id}`])
        .filter((x) => x.length > 0)
        .map((x) => parseFloat(x));
...
export function getColumns(
  ...
  const tileColumns = tiles
    .filter((t) => !["PREDICTION", "LEARNING_OUTCOMES"].includes(t.content))
    .map((t) =>
      getColumn(
        t,
        tileEntries.filter((e) => e.tile_id === t.id),
        averaged,
      ),
    )
...
function getEntryKey(entry: TileEntry, tiles: Tile[]): string {
  ...
  switch (tile.content) {
    case "BINARY":
      return entry.tile_id.toString();
    default:
      return `${entry.tile_id}_${entry.id}`;
  }
}

function tileToColumnData(
  t: Tile,
  entries: TileEntry[],
  ...
  if (t.type === "DISCUSSIONS") {
    return tileEntries.map((e) => {
      const entry_discussions = discussions.filter((disc) => {
        return (
          disc.title === e.title &&
          (disc.posted_by === student.name ||
            disc.posted_by === student.userID.toString())
        );
      });
